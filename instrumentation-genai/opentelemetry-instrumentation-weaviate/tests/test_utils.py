...
class WeaviateSpanTestBase(TestBase):
    ...
    def assert_parent_child_relationship(self, parent_span: Span, child_span: Span):
        ...
        self.assertEqual(child_span.parent.trace_id, parent_span.context.trace_id)
    
    def assert_span_properties(self, span: Span, expected_name: str, expected_kind: SpanKind = SpanKind.CLIENT):
    ...
    def assert_db_attributes(self, span: Span, operation_name: str = None):
        ...
        if operation_name:
            self.assertEqual(attributes[DbAttributes.DB_OPERATION_NAME], operation_name)
    
    def assert_server_attributes(self, span: Span, host: str = None, port: int = None):
    ...
    def get_child_spans(self, spans: List[Span], parent_span: Span) -> List[Span]:
        """Get all child spans of the given parent span."""
        return [
            span for span in spans 
            if span.parent and span.parent.span_id == parent_span.context.span_id
        ]
